...
class FakePaperQABackend:
    ...
    async def retrieve(
        ...
        *,
        ...
        scored = [
            (lexical_score(query, record), record.chunk.chunk_id)
        ...
        return tuple(
            PaperQABackendMatch(
                ...
                score=score,
                contextual_summary=(
                    f"Offline context for {chunk_id}" if contextualize else None
                ),
            )
...
class NativePaperQABackend:
    ...
    async def retrieve(
        ...
        *,
        ...
        for version_id in sorted(records_by_version):
            ...
            texts = [
                paperqa.Text(
                    ...
                    name=f"odr_chunk_{record.chunk.chunk_id[-16:]}",
                    doc=doc,
        ...
        seen: set[str] = set()
        for rank, text in enumerate(matches, start=1):
            ...
            if chunk_id in seen:
                continue
            seen.add(chunk_id)
            # The public retrieve_texts API preserves rank but discards raw MMR scores.
            results.append(
                PaperQABackendMatch(chunk_id=chunk_id, score=1.0 / rank)
            )
    ...
    async def search(
        ...
        *,
        ...
        records = eligible_records(
            await self._catalog.list_records(access, scope), request
        )
        if not records:
