...
async def get_all_tools(config: RunnableConfig):
    """
    整体注释解析：组装 Researcher 可用的完整工具箱

    这个函数负责根据当前运行配置，动态组装 researcher 阶段可以使用的全部工具。

    它会依次加载三类工具：
    1. 核心研究控制工具：
       - ResearchComplete：用于表示当前子研究任务完成；
       - think_tool：用于让 researcher 进行阶段性反思和策略规划。

    2. 搜索工具：
       - 根据配置中的 search_api 决定加载哪类搜索工具；
       - 可能是 Tavily、OpenAI 原生搜索、Anthropic 原生搜索，或不加载搜索工具。

    3. MCP 工具：
       - 如果配置了 MCP server，则加载外部 MCP tools；
       - 同时会检查已有工具名称，避免 MCP 工具和内置工具重名冲突。

    输入：
    - config: RunnableConfig，包含 search_api、mcp_config、mcp_prompt 等运行配置。

    输出：
    - tools: list
      返回 researcher 可绑定到模型上的完整工具列表。

    关键设计点：
    - 工具不是写死的，而是由 Configuration 动态决定；
    - 内置工具、搜索工具、MCP 工具会被合并到同一个 tools 列表；
    - existing_tool_names 用于防止 MCP 工具覆盖已有工具；
    - 这个函数通常被 researcher(...) 和 researcher_tools(...) 调用。
    """
    ...
    # 先添加 researcher 必备的核心工具。
    #
    # tool(ResearchComplete)：
    # - 将 ResearchComplete 包装成 LangChain 可调用工具；
    # - researcher 可以通过调用它来声明当前子研究任务完成；
    # - 后续 researcher_tools 会检测是否调用了 ResearchComplete，
    #   如果调用，则进入 compress_research 阶段。
    #
    # think_tool：
    # - 用于显式记录 researcher 的策略思考；
    # - 例如判断当前搜索是否充分、下一步应该查什么、是否需要换关键词等。
    ...
    #
    # 这里主要使用 configurable.search_api，
    # 用它决定应该加载哪一种搜索工具。
    configurable = Configuration.from_runnable_config(config)

    # 获取搜索 API 类型。
    #
    # get_config_value(...) 通常用于把配置值转换成底层真实值。
    #
    # 例如 configurable.search_api 可能是：
    # - SearchAPI.TAVILY
    # - "tavily"
    #
    # SearchAPI(...) 会进一步把字符串包装成 SearchAPI 枚举值，
    # 确保后续 get_search_tool(...) 拿到的是标准枚举。
    search_api = SearchAPI(get_config_value(configurable.search_api))

    # 根据 search_api 异步加载对应搜索工具。
    #
    # 可能返回：
    # - Tavily 搜索工具；
    # - OpenAI 原生 web search 工具封装；
    # - Anthropic 原生 web search 工具封装；
    # - 空列表，表示不启用搜索工具。
    ...
    # =========================
    # Step 3：收集已有工具名称，避免 MCP 工具名冲突
    # =========================

    # 记录当前已经存在的工具名称。
    #
    # 为什么要做这一步？
    # - 后续 MCP server 可能也暴露工具；
    # - 如果 MCP 工具名称和内置工具或搜索工具重名，
    #   模型调用时会产生歧义；
    # - 因此需要把已有工具名传给 load_mcp_tools(...) 做冲突检查。
    #
    # 普通 LangChain Tool 通常有 .name 属性；
    # 某些工具可能是 dict，因此使用 tool.get("name", "web_search") 兜底。
    existing_tool_names = {
    ...
    # =========================
    # Step 4：加载 MCP 工具，并合并到工具箱
    # =========================

    # 根据配置加载 MCP tools。
    #
    # load_mcp_tools(...) 通常会：
    # - 检查 configurable.mcp_config 是否存在；
    # - 连接 MCP server；
    # - 获取可用工具列表；
    # - 根据 existing_tool_names 过滤或处理重名工具；
    # - 返回可绑定到模型的 MCP 工具对象。
    mcp_tools = await load_mcp_tools(config, existing_tool_names)

    # 将 MCP 工具加入总工具列表。
    #
    # 最终 tools 就是 researcher 可用的完整工具集合。
